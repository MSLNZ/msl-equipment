    ...
    def set_units(self, desired_units: dict[str, str]) -> None:
        ...
        for quantity, u in desired_units.items():
            if quantity not in allowed_quantities_units:
                self.raise_exception(f'{quantity} is not a known quantity on this {self.info["Model"]} device. '
            ...
            if u in allowed_quantities_units[quantity]:
                ...
                for i in range(5 + len(self._pressure_modules)):
                    reply = self.read()
                    list_unit = [_u.strip() for _u in reply.split(":")]
                    self.units[list_unit[0]] = list_unit[1]
                if quantity in self.units:
                    ...
                    self.raise_exception(f"{quantity} unit not set correctly. Units are {self.units}")
            else:
                self.raise_exception(f"{u} is not an allowed unit. Please select from: {allowed_quantities_units[quantity]}.")
