...
from .picoscope_function_pointers import ps6000Api_funcptrs
...
class PicoScope6000(PicoScope):
    ...
    def get_values_bulk_asyc(self, start_index, down_sample_ratio, down_sample_ratio_mode, from_segment_index, to_segment_index):
        no_of_samples = c_uint32()
        overflow = c_int16()
        self.GetValuesBulkAsyc(self._handle, start_index, byref(no_of_samples), down_sample_ratio, down_sample_ratio_mode, from_segment_index, to_segment_index, byref(overflow))
        return no_of_samples.value, overflow.value
    ...
    def set_channel(self, channel, enabled, type, range, analogue_offset, bandwidth):
        self.SetChannel(self._handle, channel, enabled, type, range, analogue_offset, bandwidth)

    def set_data_buffer(self, channel, buffer_lth, down_sample_ratio_mode):
        buffer = c_int16()
        self.SetDataBuffer(self._handle, channel, byref(buffer), buffer_lth, down_sample_ratio_mode)
        return buffer.value

    def set_data_buffer_bulk(self, channel, buffer_lth, waveform, down_sample_ratio_mode):
        buffer = c_int16()
        self.SetDataBufferBulk(self._handle, channel, byref(buffer), buffer_lth, waveform, down_sample_ratio_mode)
        return buffer.value

    def set_data_buffers(self, channel, buffer_lth, down_sample_ratio_mode):
        buffer_max = c_int16()
        buffer_min = c_int16()
        self.SetDataBuffers(self._handle, channel, byref(buffer_max), byref(buffer_min), buffer_lth, down_sample_ratio_mode)
    ...
    def set_data_buffers_bulk(self, channel, buffer_lth, waveform, down_sample_ratio_mode):
        buffer_max = c_int16()
        buffer_min = c_int16()
        self.SetDataBuffersBulk(self._handle, channel, byref(buffer_max), byref(buffer_min), buffer_lth, waveform, down_sample_ratio_mode)
        return buffer_max.value, buffer_min.value
    ...
    def set_external_clock(self, frequency, threshold):
        self.SetExternalClock(self._handle, frequency, threshold)
    ...
    def set_sig_gen_properties_arbitrary(self, offset_voltage, pk_to_pk, start_delta_phase, stop_delta_phase, delta_phase_increment, dwell_count, sweep_type, shots, sweeps, trigger_type, trigger_source, ext_in_threshold):
        self.SetSigGenPropertiesArbitrary(self._handle, offset_voltage, pk_to_pk, start_delta_phase, stop_delta_phase, delta_phase_increment, dwell_count, sweep_type, shots, sweeps, trigger_type, trigger_source, ext_in_threshold)

    def set_sig_gen_properties_built_in(self, offset_voltage, pk_to_pk, start_frequency, stop_frequency, increment, dwell_time, sweep_type, shots, sweeps, trigger_type, trigger_source, ext_in_threshold):
        self.SetSigGenPropertiesBuiltIn(self._handle, offset_voltage, pk_to_pk, start_frequency, stop_frequency, increment, dwell_time, sweep_type, shots, sweeps, trigger_type, trigger_source, ext_in_threshold)
    ...
    def set_waveform_limiter(self, n_waveforms_per_second):
        self.SetWaveformLimiter(self._handle, n_waveforms_per_second)
