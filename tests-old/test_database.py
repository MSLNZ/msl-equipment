...
def test_database():
    ...
    assert (
        len(dbase.records(manufacturer="^Ag", connection=True)) == 3
    )  # all records from Agilent with a ConnectionRecord
    ...
    assert len(dbase.connections(interface=constants.Interface.SDK)) == 2
    assert len(dbase.connections(interface="SERIAL|SDK")) == 4
    assert (
        len(dbase.connections(interface=constants.Interface.SERIAL)) == 2
    )  # != 3 since "Coherent Scientific" uses PyVISA
...
def test_encoding():
    ...
    for cfg in ["utf8_txt.xml", "cp1252_txt.xml", "xlsx.xml"]:
        ...
        assert db.records(manufacturer="Kepco")[0].manufacturer == "Kepco and \u201cTMK\u201d shunt"
        assert db.records(model="MFF101/M")[0].description == "Motorized Filter Flip Mount for \xd825mm Optics"
...
def test_json_and_xml_db():
    for filename in ["config_json.xml", "config_xml.xml"]:
        ...
        assert m.calibration.expanded_uncertainty == (
            (0, 0, 0, 0.017),
            (0, 0, 0, 0.005),
            (0, 0, 0, 0.002),
            (0, 0, 0, 0.002),
