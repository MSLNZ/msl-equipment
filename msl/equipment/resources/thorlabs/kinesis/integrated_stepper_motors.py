...
class UnitType(IntEnum):
    ...
    Used to :meth:`~.IntegratedStepperMotors.get_real_value_from_device_unit`
    and to :meth:`~.IntegratedStepperMotors.get_device_unit_from_real_value`.
    """
    distance = 0
    velocity = 1
    acceleration = 2
...
class IntegratedStepperMotors(MotionControl):
    ...
    def get_jog_vel_params(self):
        ...
        return acceleration.value, max_velocity.value

    def set_jog_vel_params(self, acceleration, max_velocity):
    ...
    def set_vel_params(self, acceleration, max_velocity):
    ...
    def set_motor_travel_mode(self, travel_mode):
        ...
        Parameters
        ----------
        travel_mode : :class:`.enum.MOT_TravelModes`
            The travel mode as a :class:`.enum.MOT_TravelModes` enum value or 
            member name.
    ...
    def set_motor_params(self, steps_per_rev, gear_box_ratio, pitch):
        ...
        ``RealWorldUnits`` (mm or degrees) (see manual). The real-world unit 
    ...
    def set_motor_velocity_limits(self, max_velocity, max_acceleration):
        ...
        max_velocity : :obj:`float`
            The maximum velocity in ``RealWorldUnits`` (see manual).
        max_acceleration : :obj:`float`
            The maximum acceleration in ``RealWorldUnits`` (see manual).
    ...
    def set_motor_travel_limits(self, min_position, max_position):
        ...
        min_position : :obj:`float`
            The minimum position in ``RealWorldUnits`` (see manual).
        max_position : :obj:`float`
            The maximum position in ``RealWorldUnits`` (see manual).
