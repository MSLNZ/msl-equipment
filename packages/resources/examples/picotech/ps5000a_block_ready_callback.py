"""Acquire PicoScope data in Block-Ready Mode using a callback function.

The callback function is notified when the data block has been acquired.
"""
...
@ps.block_ready
def run_done(handle: int, status: int, _: None) -> None:
    """Called when all samples have been acquired."""
    print(f"Callback: Data acquisition is done! {handle=}, {status=}")
    ...
    print(f"Number of samples acquired {num_acquired}")
    print(f"Overflow bit mask => {overflow_mask}")

    print(f"The time between samples is {scope.dt} seconds")
    print("The voltages are:")
    print(scope.channel["A"].volts)


print("Example :: Using block-ready mode with a callback function")
...
# Pass in our callback function to be notified when data acquisition is finished
wait_time = scope.run_block(callback=run_done)
