...
from msl.equipment.dns_service_discovery import (
    _DNSRecord,  # pyright: ignore[reportPrivateUsage]
    _DomainNamePointer,  # pyright: ignore[reportPrivateUsage]
    _HostAddress,  # pyright: ignore[reportPrivateUsage]
    _QType,  # pyright: ignore[reportPrivateUsage]
    _Question,  # pyright: ignore[reportPrivateUsage]
    _ResourceRecord,  # pyright: ignore[reportPrivateUsage]
...
def test_dns_record1() -> None:
    ...
    assert dns.questions == [
        _Question(q_name="_vxi-11._tcp.local.", q_type=12, q_class=1),
        _Question(q_name="_hislip._tcp.local.", q_type=12, q_class=1),
        _Question(q_name="_http._tcp.local.", q_type=12, q_class=1),
        _Question(q_name="_lxi._tcp.local.", q_type=12, q_class=1),
        _Question(q_name="_scpi-raw._tcp.local.", q_type=12, q_class=1),
        _Question(q_name="_scpi-telnet._tcp.local.", q_type=12, q_class=1),
    ]
    assert dns.answers == [
        _ResourceRecord(
            "_http._tcp.local.",
            ...
            1,
            10,
            _DomainNamePointer("Manufacturer 0 Product Number - 0._http._tcp.local."),
        ),
        _ResourceRecord(
            "_lxi._tcp.local.",
            _QType(12),
            1,
            10,
            _DomainNamePointer("Manufacturer 0 Product Number - 1._lxi._tcp.local."),
        ),
        _ResourceRecord(
            ...
            1,
            10,
            _DomainNamePointer("Manufacturer 1 Product Number - 0._scpi-raw._tcp.local."),
        ),
        _ResourceRecord(
            ...
            1,
            10,
            _DomainNamePointer("Manufacturer 2 Product Number - 7._scpi-telnet._tcp.local."),
        ),
        _ResourceRecord(
            "_vxi-11._tcp.local.",
            _QType(12),
            1,
            10,
            _DomainNamePointer("Manufacturer 9 Product Number - 9._vxi-11._tcp.local."),
        ),
        _ResourceRecord(
            "_hislip._tcp.local.",
            _QType(12),
            1,
            10,
            _DomainNamePointer("Manufacturer 6 Product Number - 2._hislip._tcp.local."),
        ),
    ]
    assert len(dns.authority) == 0
    assert dns.additional == [
        _ResourceRecord(
            "Manufacturer 0 Product Number - 0._http._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 80, "X-123456-0000-0.local."),
        ),
        _ResourceRecord(
            "Manufacturer 0 Product Number - 0._http._tcp.local.",
            _QType(16),
            1,
            10,
            _Text({"txtvers": "1", "path": "/"}),  # cSpell: ignore txtvers
        ),
        _ResourceRecord(
            "Manufacturer 0 Product Number - 1._lxi._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 80, "X-123456-0000-0.local."),
        ...
        _ResourceRecord(
            "Manufacturer 1 Product Number - 0._scpi-raw._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 5025, "X-123456-0000-0.local."),
        ),
        _ResourceRecord(
            "Manufacturer 1 Product Number - 0._scpi-raw._tcp.local.",
            _QType(16),
        ...
        _ResourceRecord(
            "Manufacturer 2 Product Number - 7._scpi-telnet._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 5024, "X-123456-0000-0.local."),
        ),
        _ResourceRecord(
            "Manufacturer 2 Product Number - 7._scpi-telnet._tcp.local.",
            _QType(16),
        ...
        _ResourceRecord(
            "Manufacturer 9 Product Number - 9._vxi-11._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 111, "X-123456-0000-0.local."),
        ),
        _ResourceRecord(
            "Manufacturer 9 Product Number - 9._vxi-11._tcp.local.",
            _QType(16),
            ...
            _Text(
                {
                    ...
                    "Model": "tuvwxy",  # cSpell: ignore tuvwxy
        ...
        _ResourceRecord(
            "Manufacturer 6 Product Number - 2._hislip._tcp.local.",
            _QType(33),
            1,
            10,
            _Service(0, 0, 4880, "X-123456-0000-0.local."),
        ),
        _ResourceRecord(
            "Manufacturer 6 Product Number - 2._hislip._tcp.local.",
            _QType(16),
        ...
        _ResourceRecord("X-123456-0000-0.local.", _QType(1), 1, 10, _HostAddress((169, 254, 100, 15))),
    ]


def test_dns_record2() -> None:
    reply = (
        b"\x00\x00\x84\x00\x00\x00\x00\x02\x00\x00\x00\x00\x05_http\x04_tcp\x05local"
        b"\x00\x00\x0c\x00\x01\x00\x00\x0e\x10\x001\x1eabcdefghijklmnopqrstuvwxyz-123"  # cSpell: ignore eabcdefghijklmnopqrstuvwxyz  # noqa: E501
    ...
    assert dns.answers == [
        _ResourceRecord(
            ...
            1,
            3600,
            _DomainNamePointer("abcdefghijklmnopqrstuvwxyz-123._http._tcp.local."),
        ),
        _ResourceRecord(
            ...
            1,
            3600,
            _DomainNamePointer("_http._tcp.local."),
        ),
    ]
    assert len(dns.authority) == 0
