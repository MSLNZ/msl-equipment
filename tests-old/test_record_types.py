...
def test_equipment_record():
    ...
    record = EquipmentRecord(
        user_defined={"x": 1, "y": 2, "z": 3}, unknown_attribute="AAA", dictionary={"a": 1, "b": 2}
    )
...
def test_is_calibration_due():
    ...
    record = EquipmentRecord(
        is_operable=True, calibrations=[CalibrationRecord(calibration_date=today, calibration_cycle=5)]
    )
    assert not record.is_calibration_due()
    ...
    record = EquipmentRecord(
        is_operable=True, calibrations=[CalibrationRecord(calibration_date=date, calibration_cycle=1.5)]
    )
    ...
    record = EquipmentRecord(
        is_operable=True, calibrations=[CalibrationRecord(calibration_cycle=1.5, report_date=rep_date)]
    )
...
def test_next_calibration_date():
    ...
    record = EquipmentRecord(
        is_operable=False, calibrations=[CalibrationRecord(calibration_date=datetime.date(2000, 1, 1))]
    )
    assert record.next_calibration_date() is None

    # operable but the calibration_cycle is not defined
    record = EquipmentRecord(
        is_operable=True, calibrations=[CalibrationRecord(calibration_date=datetime.date(2000, 1, 1))]
    )
    assert record.next_calibration_date() is None
    ...
    record = EquipmentRecord(
        ...
        calibrations=[
            CalibrationRecord(
                calibration_date=datetime.date(2000, 3, 14),
                calibration_cycle=0.7,
...
def test_calibration_record():
    ...
    cr = CalibrationRecord(
        calibration_cycle=5,
        calibration_date="2018-02-24",
        measurands=measurands,
        report_date="2010-12-13",
        report_number="ABC123",
    )
