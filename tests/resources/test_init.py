...
def test_find_resource_class():
    ...
    for man in ('OMEGA', 'omega'):
        for suffix in ('w3', 'd3', 'sd', 'm', 'w', '2'):
            ...
            cls = resources.find_resource_class(record)
            assert cls is resources.omega.ithx.iTHX
    ...
    for man in ('Pico Tech', 'Pico Technologies', 'Pico Technology'):
        for mod in ('PicoScope 2104', '2104', '2105', '2202', '2203', '2204', '2205', '2204A', '2205A'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps2000.PicoScope2000, mod
        for mod in ('PicoScope 2205A MSO', '2205A MSO', '2205 MSO',
            ...
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps2000a.PicoScope2000A, mod
        for mod in ('PicoScope 3204', '3204', '3205', '3206', '3224', '3424', '3425'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps3000.PicoScope3000, mod
        for mod in ('PicoScope 3203D', '3203D', '3204D', '3205D', '3206D',
            ...
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps3000a.PicoScope3000A, mod
        for mod in ('PicoScope 4224', '4224', '4224 IEPE', '4262', '4424'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps4000.PicoScope4000, mod
        for mod in ('PicoScope 4444', '4444', '4824'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps4000a.PicoScope4000A, mod
        for mod in ('PicoScope 5000', '5000'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps5000.PicoScope5000, mod
        for mod in ('PicoScope 5242A', '5242A', '5243A', '5244A', '5442A', '5443A', '5444A',
                    '5242B', '5243B', '5244B', '5442B', '5443B', '5444B'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps5000a.PicoScope5000A, mod
        for mod in ('PicoScope 6407', '6407', '6402C', '6402D', '6403C', '6403D', '6404C', '6404D'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.picoscope.ps6000.PicoScope6000, mod
        for mod in ('PT-104', 'PT104'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.picotech.pt104.PT104, mod

    for man in ('Thorlabs', 'Thorlabs Inc.'):
        for mod in ('FW102C', 'FW102CNEB', 'FW212C', 'FW212CNEB'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.fwxx2c.FilterWheelXX2C
        for mod in ('BSC101', 'BSC102', 'BSC103', 'BSC201', 'BSC202', 'BSC203'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.benchtop_stepper_motor.BenchtopStepperMotor
        for mod in ('MFF101', 'MFF102', 'MFF101/M', 'MFF102/M'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.filter_flipper.FilterFlipper
        for mod in ('LTS150', 'LTS150/M', 'LTS300', 'LTS300/M',
                    'MLJ050/M', 'MLJ150', 'MLJ150', 'MLJ150/M'
                    'K10CR1', 'K10CR1/M'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.integrated_stepper_motors.IntegratedStepperMotors
        for mod in ('KDC101', ):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.kcube_dc_servo.KCubeDCServo
        for mod in ('KSC101', ):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.kcube_solenoid.KCubeSolenoid
        for mod in ('KST101', ):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.thorlabs.kinesis.kcube_stepper_motor.KCubeStepperMotor
    ...
    for man in ('OptoSigma', 'Opto Sigma', 'SigmaKoki', 'SigmaKoki', 'Sigma Koki Co. LTD'):
        ...
        cls = resources.find_resource_class(record)
        assert cls is resources.optosigma.shot702.SHOT702
    ...
    for man in ('Optronic Laboratories', 'Optronic Laboratories, Inc.',
        ...
        for mod in ('756', 'OL756', 'OL 756'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.optronic_laboratories.OL756
    ...
    for man in ('Energetiq', 'ENERGETIQ', 'Energetiq Technology, Inc.',
        ...
        for mod in ('eq-99', 'EQ-99', 'eQ-99-MgR', 'EQ-99-MGR'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.energetiq.eq99.EQ99
    ...
    for man in ('Raicol', 'Raicol Crystals', 'Raicol Crystals Ltd.',
        ...
        for mod in ('TEC', 'TEC20-60', 'TEC 20 - 60'):
            record = ConnectionRecord(manufacturer=man, model=mod, backend=Backend.MSL)
            cls = resources.find_resource_class(record)
            assert cls is resources.raicol.RaicolTEC
