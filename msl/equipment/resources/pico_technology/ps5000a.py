...
from .picoscope_function_pointers import ps5000aApi_funcptrs
...
class PicoScope5000A(PicoScope):
    ...
    def get_device_resolution(self):
        resolution = c_enum()
        self.GetDeviceResolution(self._handle, byref(resolution))
    ...
    def open_unit(self, resolution):
        handle = c_int16()
        serial = c_int8()
        self.OpenUnit(byref(self._handle), byref(serial), resolution)
        return handle.value, serial.value

    def open_unit_async(self, resolution):
        status = c_int16()
        serial = c_int8()
        self.OpenUnitAsync(byref(status), byref(serial), resolution)
    ...
    def set_device_resolution(self, resolution):
        self.SetDeviceResolution(self._handle, resolution)
